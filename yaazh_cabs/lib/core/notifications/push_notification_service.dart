...
import 'package:yaazh_cabs/app/router.dart';
...
class PushNotificationService {
    ...
    try {
      ...
      if (token != null && token.isNotEmpty) {
        await _ref
            ...
            .registerToken(token, appVersion: '1.0.0');
      }

      messaging.onTokenRefresh.listen((next) {
        _ref
            .read(deviceServiceProvider)
            .registerToken(next, appVersion: '1.0.0');
      });
  ...
  void _onForeground(RemoteMessage message) {
    final title =
        message.notification?.title ?? message.data['title'] ?? 'Trip update';
    ...
    ScaffoldMessenger.of(ctx).showSnackBar(
      SnackBar(
        ...
        action: location == null
            ...
            : SnackBarAction(
                label: 'View',
                onPressed: () => GoRouter.of(ctx).go(location),
              ),
      ),
    );
  }
  ...
  String? _locationFrom(RemoteMessage message) {
    ...
    if (bookingId != null && bookingId.toString().isNotEmpty) {
      if (type.contains('started') || type.contains('on_trip')) {
        return '/active-trip/${bookingId.toString()}';
      }
