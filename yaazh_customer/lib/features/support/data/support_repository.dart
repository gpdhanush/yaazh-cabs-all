...
class SupportRepository {
  ...
  Future<List<SupportTicket>> list() async {
    final data = await _api.get('/customer/support');
  ...
  Future<String> create({required String subject, required String message}) async {
    final data = await _api.post('/customer/support', data: {
      'subject': subject,
      'message': message,
    });
  ...
  Future<SupportDetail> get(String id) async {
    final data = await _api.get('/customer/support/$id');
  ...
  Future<void> sendMessage(String id, String message) async {
    await _api.post('/customer/support/$id/messages', data: {'message': message});
  }
}
