...
For the `Backend-node` API on cPanel, upload the current source plus `package.json` and `package-lock.json`, run `npm ci --omit=dev` in the `Backend-node` directory using Node Selector/SSH, and restart the application. This is required after adding dependencies such as `firebase-admin`, `nodemailer`, `multer`, and `pdfkit`; otherwise the Node process will fail with `Cannot find module 'firebase-admin'` before the API can respond.
