# Elevate Ride

# 21st.dev Premium Animation & UI Prompt for Yaazh Cabs


cd /Users/naethra/Desktop/Projects/gp/yaaz/yaazh-web
cp .env.example .env.local
npm run dev
## Objective



Use premium components from **21st.dev** to build a world-class taxi booking website.



The website must NOT look like a traditional taxi website.



It should feel like a luxury travel company with smooth cinematic animations similar to Apple, Airbnb, Tesla and Linear.



Use only production-ready components from **21st.dev**, Tailwind CSS, Framer Motion, Lenis Smooth Scroll and modern React patterns.



---



# Animation Libraries



Use



- Tailwind CSS

- Framer Motion

- Lenis Smooth Scroll

- Motion (Framer Motion v12+)

- Lucide Icons

- React Intersection Observer



Avoid GSAP unless absolutely required.



Maintain 60 FPS animations.



---



# Hero Section



Use a **Container Scroll Animation** from 21st.dev.



Requirements



- Full-screen cinematic hero

- Transparent navbar

- Navbar becomes solid on scroll

- Background slowly zooms

- Taxi image has floating animation

- Mountains move with parallax

- Heading fades upward

- CTA buttons animate independently

- Booking form floats using glassmorphism

- Mouse movement creates subtle 3D perspective



Hero Animation



- Fade Up

- Scale

- Parallax

- Blur Reveal

- Stagger Animation



---



# Booking Form



Modern Glass Card



Use



- Glassmorphism

- Backdrop Blur

- Soft Shadow

- Floating Effect



Fields



- Pickup

- Drop

- Pickup Date

- Pickup Time

- Vehicle

- Trip Type



Requirements



- Tailwind UI

- 5px Border Radius

- Searchable Dropdown

- Modern Date Picker

- Modern Time Picker

- Past dates disabled

- Pickup Time mandatory

- Floating labels

- Skeleton loading

- Inline validation

- Loading button animation



---



# Trusted Partners



Use an Infinite Logo Marquee.



Animation



- Infinite horizontal scroll

- Pause on hover

- Gradient fade at edges



---



# Services Section



Use **Spotlight Cards** from 21st.dev.



Cards



- Airport Taxi

- One Way Taxi

- Round Trip

- Tour Packages

- Local Taxi

- Corporate Taxi



Animation



- Fade Up

- Stagger

- Scale on Hover

- Glow Border

- Icon Rotation

- Lift Card

- Ripple Hover



---



# Popular Routes



Use Horizontal Scroll Cards.



Routes



- Udumalpet → Pollachi

- Udumalpet → Coimbatore

- Udumalpet → Palani

- Pollachi → Coimbatore



Card Animation



- Slide Left

- Scale

- Gradient Border

- Image Zoom

- Magnetic Button



---



# Fleet Section



Use Premium Vehicle Cards.



Vehicles



- Dzire

- Ertiga

- Innova

- SUV

- Tempo Traveller



Animation



- Fade Up

- Vehicle Floating

- Tilt on Hover

- Image Zoom

- Shadow Glow



---



# Fare Estimator



Create a premium fare calculator.



Animation



- Number Count

- Price Morph

- Smooth Value Change



When vehicle changes



- Fare animates smoothly



---



# Booking Process



Use **Radial Orbital Timeline** from 21st.dev.



Steps



1 Select Route



↓



2 Choose Vehicle



↓



3 Confirm Booking



↓



4 Receive Confirmation



↓



5 Enjoy Ride



Animation



- Connecting Line Animation

- Step Zoom

- Active Glow

- Smooth Progress



---



# Statistics



Use Animated Counter.



Items



10000+



Happy Customers



500+



Daily Trips



25+



Professional Drivers



24x7



Support



Animation



- Count Up

- Icon Bounce

- Fade

- Glow



---



# Testimonials



Use Testimonials Columns.



Animation



- Infinite Auto Scroll

- Fade

- Pause on Hover

- Glass Cards



---



# Gallery



Use Masonry Grid.



Animation



- Fade

- Scale

- Stagger

- Lightbox

- Hover Zoom



---



# FAQ



Accordion



Animation



- Smooth Expand

- Rotate Arrow

- Fade Content



---



# Footer



Use Motion Footer.



Animation



- Fade Up

- Icons Rotate

- Link Underline Animation



---



# Global Scroll Animation



Every section must animate only when entering the viewport.



Use



- whileInView

- viewport once

- staggerChildren

- delayChildren



Never animate everything immediately.



---



# Hover Effects



Buttons



- Magnetic

- Scale

- Glow

- Ripple



Cards



- Lift

- Tilt

- Shadow

- Gradient Border



Images



- Scale

- Float

- Blur to Sharp



Icons



- Rotate

- Bounce

- Glow



---



# Cursor Effects



Modern Cursor



- Cursor Blob

- Hover Magnet

- Expand on Buttons

- Shrink on Links



---



# Page Transition



Between pages



Use



- Fade

- Scale

- Blur

- Slide



Duration



300–500ms



---



# Loading Experience



Skeleton Loader



Animated Progress



Shimmer Cards



Image Blur Placeholder



---



# Mobile Experience



Animations should remain smooth.



Reduce heavy effects on low-end devices.



Respect prefers-reduced-motion.



---



# Performance Rules



Maintain



- Lighthouse Performance 95+

- SEO 100

- Accessibility 95+

- Best Practices 100



Use



- Lazy Loading

- Dynamic Imports

- Server Components

- Optimized Images

- WebP/AVIF

- Code Splitting



---



# UI Rules



- Tailwind CSS only

- No Bootstrap

- No jQuery

- No template layouts

- Premium spacing

- Soft shadows

- Glassmorphism

- Rounded corners

- Clean typography

- Apple-level polish



---



# Final AI Instruction



Build an award-winning luxury taxi booking website using the best production-ready components from **21st.dev**. Prioritize **Container Scroll Hero**, **Spotlight Cards**, **Horizontal Scroll Route Cards**, **Premium Fleet Cards**, **Radial Orbital Timeline**, **Animated Counters**, **Testimonials Columns**, **Masonry Gallery**, and **Motion Footer**. Use Tailwind CSS, Framer Motion, and Lenis Smooth Scroll. Every section should have unique scroll-triggered animations, glassmorphism, magnetic buttons, subtle 3D hover effects, parallax backgrounds, and smooth page transitions. The website must feel handcrafted by a senior UI/UX designer and frontend engineer, delivering an Apple-quality experience rather than a generic taxi booking template.

## Development

Prefer working locally? You need Node.js and npm — [install with nvm](https://github.com/nvm-sh/nvm#installing-and-updating).

```sh
git clone <this-repository-url>
cd <repository-name>
npm i
npm run dev
```
